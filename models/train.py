...
if __name__ == '__main__':
    ...
    def _load_data(particle, datafile):
        ...
        # convert to MeV
        energy = d['energy'][:].reshape(-1, 1) * 1.0
    ...
    sizes = sizes[:5].tolist()

    # scale the energy depositions by 1000 to convert MeV => GeV
    first, second, energy = [
        (X.astype(np.float32) / 1000)
        for X in [first, second, energy]
    ]

    le = LabelEncoder()
    y = le.fit_transform(y)

    first, second, energy = shuffle(first, second, energy,
                                              random_state=0)
    ...
    for epoch in range(nb_epochs):
        ...
        for index in range(nb_batches):
            ...
            # we do this twice simply to match the number of batches per epoch
